    ...
    <>
      ...
      <section className="relative h-[75vh] md:h-[90vh] bg-neutral-900 text-white overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src={heroHouse} alt="Hero custom home exterior" className="w-full h-full object-cover object-center brightness-[0.95]" />
          <div className="absolute inset-0 bg-black/20 z-10" />
        </div>
        <div className="container mx-auto px-4 md:px-8 relative z-20 flex flex-col justify-center h-full">
          <motion.h1 className="text-4xl md:text-5xl lg:text-6xl font-bold max-w-3xl" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            Custom Homes.<br />Family Values.<br />Builder-Direct.
          </motion.h1>
          <motion.p className="text-xl text-white/90 mt-6 max-w-2xl" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
          ...
          <motion.div className="mt-8 flex flex-wrap gap-4" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
            <Link to="/contact" className="btn btn-primary text-lg px-8 py-4">Start Your Dream Build</Link>
            <Link to="/portfolio" className="btn btn-outline text-white border-white hover:bg-white hover:text-neutral-900 text-lg px-8 py-4">View Our Work</Link>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 md:px-8">
            ...
            {[{
              ...
              <Link key={idx} to={project.link} className="relative overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow group">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent p-6 flex flex-col justify-end">
                  <span className="text-sm font-semibold text-white bg-primary-600 px-3 py-1 rounded mb-2 w-fit">{project.category}</span>
          ...
          <div className="mt-12 text-center">
            <Link to="/portfolio" className="inline-block px-8 py-3 border border-primary-600 text-primary-600 font-semibold rounded hover:bg-primary-600 hover:text-white transition">
              View All Projects
            </Link>
          </div>
        </div>
